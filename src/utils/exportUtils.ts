 ...
 */
export function exportFinancialsCSV(company: Company, jobs: CleaningJob[], expenses: Expense[]) {
  const headers = ['Type', 'Date', 'Description / Client', 'Category / Service', 'Amount (£)', 'Status'];
  ...
  expenses.forEach((e) => {
    rows.push(['EXPENSE', e.date, e.description, e.category, -e.amount, 'PAID']);
  });
