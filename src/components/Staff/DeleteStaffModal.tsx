...
export const DeleteStaffModal: React.FC<DeleteStaffModalProps> = ({ isOpen, onClose, staffUser }) => {
  const { users, jobs, deleteUser, reassignUserJobs, currentUser, language } = useApp();
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-lg text-slate-900 dark:text-white">
                {getTranslation(language, 'deleteStaffModalTitle')}
              </h3>
              <p className="text-xs text-red-600 dark:text-red-400 font-medium">
                {getTranslation(language, 'deleteStaffModalSub')}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
            ...
            <div className="p-4 bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 rounded-2xl space-y-3">
              <div className="flex items-start gap-2.5 text-amber-800 dark:text-amber-200">
                ...
                <div>
                  <h5 className="font-extrabold text-xs">
                    {futureJobs.length} {getTranslation(language, 'scheduledCleansFound')}
                  </h5>
                  <p className="text-[11px] text-amber-700 dark:text-amber-300 mt-0.5">
                    {getTranslation(language, 'staffHasOpenJobsMsg')}
                  </p>
                </div>
              </div>
              ...
              <div>
                <label className="text-xs font-bold text-amber-900 dark:text-amber-100 flex items-center gap-1.5 mb-1">
                  <UserCheck className="w-3.5 h-3.5 text-amber-600" /> {getTranslation(language, 'reassignJobsPrompt')}
                </label>
                <select
                ...
                >
                  <option value="">{getTranslation(language, 'noReassignOption')}</option>
          ...
          <div className="pt-3 flex justify-end gap-2 border-t border-slate-100 dark:border-slate-700">
            ...
            >
              ...
              {getTranslation(language, 'confirmDeleteStaffBtn')}
            </button>
          </div>
        </div>
      </div>
