...
const ScheduleViewComponent: React.FC<ScheduleViewProps> = ({
  ...
  // Period filter state for metrics panel
  const [metricsPeriod, setMetricsPeriod] = useState<'TODAY' | 'WEEK' | 'MONTH' | 'CUSTOM'>('TODAY');
  ...
  const [editCleanerIds, setEditCleanerIds] = useState<string[]>([]);
  ...
  const filterJobs = useCallback(
    (jobList: CleaningJob[]) => {
      return jobList.filter((j) => {
        ...
        if (cleanerFilter !== 'ALL') {
          ...
          const isAssigned = assigned.some((u) => u.id === cleanerFilter) || j.cleanerId?.includes(cleanerFilter);
    ...
    [statusFilter, cleanerFilter, searchQuery, users]
  );
  ...
  const periodJobs = useMemo(() => {
    if (metricsPeriod === 'TODAY') {
      ...
      const accumulatedJobs: CleaningJob[] = [];
  ...
  const targetCollaborator = useMemo(() => {
    if (cleanerFilter !== 'ALL') {
      return users.find((u) => u.id === cleanerFilter) || currentUser;
    }
    return currentUser;
  }, [cleanerFilter, users, currentUser]);

  // Collaborator metrics using calculateUserMetrics & discounting payrollPayments
  const collaboratorMetrics = useMemo(() => {
    return calculateUserMetrics(targetCollaborator, periodJobs, users, payrollPayments);
  }, [targetCollaborator, periodJobs, users, payrollPayments]);

  // Real-time recalculated Intelligent Metrics for company or route calculation
  const selectedDateMetrics = useMemo(() => {
    const activeJobs = periodJobs.filter((j) => j.status !== 'CANCELLED');
    ...
    const periodExpenses = (expenses || [])
      .filter((e) => {
        if (e.companyId !== currentCompany.id) return false;
        if (metricsPeriod === 'TODAY') return e.date === selectedDate;
        if (metricsPeriod === 'WEEK') {
          const weekDates = new Set(weekDays.map((w) => w.dateStr));
          return weekDates.has(e.date);
        }
    ...
    const netProfit = totalRevenue - staffCosts - periodExpenses;
    const profitMargin = totalRevenue > 0 ? (netProfit / totalRevenue) * 100 : 0;

    // Route calculation for rawSelectedDateJobs (for today/selected date route)
    const routeActiveJobs = rawSelectedDateJobs.filter((j) => j.status !== 'CANCELLED');
    const cleanerJobMap = new Map<string, { cleaner: any; jobs: typeof routeActiveJobs }>();
    routeActiveJobs.forEach((job) => {
    ...
    const allOrderedJobs: typeof routeActiveJobs = [];
    ...
    const routeResult = optimizeRoute(
      ...
      allOrderedJobs.length > 0 ? allOrderedJobs : routeActiveJobs
    );

    const routeList = (allOrderedJobs.length > 0 ? allOrderedJobs : routeActiveJobs).map((job) => ({
      job,
  ...
  const handleOpenEditModal = (job: CleaningJob) => {
    setEditingJob(job);
    const assigned = getAssignedStaffForJob(job, users);
    let initialIds = assigned.map((u) => u.id);
    if (initialIds.length === 0 && job.cleanerId) {
      initialIds = job.cleanerId.split(/[,;]+/).map((s) => s.trim()).filter(Boolean);
    }
    setEditCleanerIds(initialIds);
  ...
  const handleSaveEditedJob = (e: React.FormEvent) => {
    ...
    const assignedUsers = users.filter((u) => editCleanerIds.includes(u.id));
    const cleanerIdStr = editCleanerIds.join(', ');
  ...
  const handleCreateJob = (e: React.FormEvent) => {
    ...
    addJob({
      ...
      cleanerId: cleaner?.id || 'usr_waylla',
      cleanerName: cleaner?.name || 'Waylla',
      date: formData.date,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            ...
            <span className="text-xs bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-200 px-2.5 py-0.5 rounded-full font-bold">
              Agenda Única Compartilhada
            </span>
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Agenda em tempo real para toda a empresa W&J. Acompanhe todos os serviços e veja quem está atribuído.
          </p>
        </div>
          ...
          {userRole !== 'CLEANER' && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-2 bg-red-600/10 hover:bg-red-600/20 text-red-600 dark:text-red-400 font-bold text-xs rounded-xl flex items-center gap-1.5 border border-red-200 dark:border-red-900/50 shadow-xs transition-colors"
                title="Apagar todos os agendamentos da agenda"
              >
      ...
      <div className="bg-slate-900 dark:bg-slate-950 text-white p-5 rounded-3xl shadow-xl border border-slate-800 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-sm tracking-tight text-white flex items-center gap-2">
                {userRole === 'OWNER' ? 'Agenda Inteligente Recalculada' : 'Seus Indicadores de Trabalho'}
                <span className="text-[10px] font-mono font-bold bg-blue-500/30 text-blue-300 px-2 py-0.5 rounded-full border border-blue-400/30">
                  {metricsPeriod === 'TODAY'
                    ? selectedDate
                    : metricsPeriod === 'WEEK'
                    ? `${weekDays[0]?.dateStr} a ${weekDays[6]?.dateStr}`
                    : metricsPeriod === 'MONTH'
                    ? monthName
              ...
              <p className="text-[11px] text-slate-300">
                {userRole === 'OWNER'
                  ...
                  : 'Horas trabalhadas e valores a receber recalculados automaticamente em tempo real.'}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            ...
            <div className="bg-slate-800/90 p-1 rounded-xl border border-slate-700/80 flex items-center gap-1 shadow-xs">
              <button
        ...
        {metricsPeriod === 'CUSTOM' && (
          <div className="flex flex-wrap items-center gap-3 bg-slate-800/60 p-2.5 rounded-2xl border border-slate-700/50">
            <span className="text-xs font-bold text-slate-300">Período Personalizado:</span>
            <div className="flex items-center gap-2">
              <input
                type="date"
                value={customStartDate}
                onChange={(e) => setCustomStartDate(e.target.value)}
                className="bg-slate-900 border border-slate-700 text-xs font-semibold text-white px-3 py-1 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
              />
        ...
        {userRole === 'OWNER' ? (
          ...
          /* ADMIN & CLEANER: 2 cards only (Horas Trabalhadas, A Receber) */
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col justify-between">
              <span className="text-xs font-bold text-slate-400 flex items-center gap-1.5">
                <Clock className="w-4 h-4 text-blue-400" /> Horas Trabalhadas
              </span>
              <div className="mt-3 flex items-baseline justify-between">
                <span className="text-2xl font-black text-blue-300 font-mono">
                  {collaboratorMetrics.totalHoursWorked.toFixed(1)}h
                </span>
            ...
            {/* Individual Unpaid Earnings (A Receber) */}
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col justify-between">
              <span className="text-xs font-bold text-slate-400 flex items-center gap-1.5">
                <PoundSterling className="w-4 h-4 text-emerald-400" /> A Receber
              </span>
              <div className="mt-3 flex items-baseline justify-between">
                <span className="text-2xl font-black text-emerald-400 font-mono">
                  £{collaboratorMetrics.totalEarnings.toFixed(2)}
                </span>
                <span className="text-[10px] text-emerald-300/80 font-bold bg-emerald-950/80 px-2 py-0.5 rounded border border-emerald-800/50">
                  Líquido a receber
                </span>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-4">
            {weekDays.map((dayItem) => {
              ...
              const dayJobsFiltered = filterJobs(rawDayJobs).sort((a, b) => a.startTime.localeCompare(b.startTime));
                    ...
                    <div className="mt-1 space-y-1 overflow-hidden">
                      {dayJobs.slice(0, 2).map((job) => {
                        ...
                        return (
                          <div
                            key={job.id}
                            title={`${job.clientName} - ${job.startTime} (${userRole !== 'CLEANER' ? `£${job.price}` : `${job.estimatedDuration}h`})`}
                            ...
                            className={`p-1 px-1.5 text-[10px] font-semibold rounded-md border transition-all flex flex-col justify-center min-w-0 ${
                              ...
                              isCompleted
                                ...
                                : isInProgress
                                ? 'bg-amber-50 text-amber-950 border-amber-300 dark:bg-amber-950/80 dark:text-amber-200 dark:border-amber-800'
                                : 'bg-blue-50 text-blue-950 border-blue-300 dark:bg-blue-950/80 dark:text-blue-200 dark:border-blue-800'
                            }`}
                          >
                            <div className="flex items-center gap-0.5 min-w-0">
                              {userRole !== 'CLEANER' && <GripVertical className="w-2.5 h-2.5 opacity-50 shrink-0" />}
                              <span className="truncate font-extrabold text-[10px] text-slate-900 dark:text-white leading-tight min-w-0 flex-1">
                                {job.clientName}
                              </span>
                            </div>
                            <div className="flex items-center justify-between text-[9px] font-mono opacity-90 leading-none mt-0.5 font-bold">
                              <span>{job.startTime}</span>
                              {userRole !== 'CLEANER' ? (
                                <span>£{job.price}</span>
                              ) : (
                                <span className="text-slate-500 dark:text-slate-400">{job.estimatedDuration}h</span>
                              )}
                            </div>
                          </div>
                        );
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {userRole !== 'CLEANER' ? (
                          ...
                          <span className="text-xs font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/60 px-2 py-0.5 rounded">
                            Sua Hora: £{(currentUser.hourlyRate ?? (userRole === 'OWNER' ? 0 : 14)).toFixed(2)}/h
                          </span>
                        )}
                    ...
                    {userRole !== 'CLEANER' && (
                      <>
                        <div className="flex flex-wrap items-center gap-1.5 max-w-[200px]">
                          {getAssignedStaffForJob(job, users).length > 0 ? (
                            getAssignedStaffForJob(job, users).map((st) => (
                              <span
                                key={st.id}
                                className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/80 text-blue-700 dark:text-blue-300 border border-blue-200/80 dark:border-blue-800"
                              >
                                {st.name}
                              </span>
                            ))
                            ...
                            <span className="text-xs text-slate-400 italic">
                              {getTranslation(language, 'unassigned')}
                            </span>
                          )}
                        </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl max-w-lg w-full border border-slate-200 dark:border-slate-700 overflow-hidden animate-in fade-in zoom-in-95 duration-150 max-h-[92vh] sm:max-h-[90vh] flex flex-col my-auto">
            ...
            <form onSubmit={handleSaveEditedJob} className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain touch-pan-y">
              ...
              <div>
                ...
                <StaffMultiSelect
                  users={users}
                  selectedIds={editCleanerIds}
                  onChange={setEditCleanerIds}
                  language={language}
                />
              </div>
      ...
      {showRouteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-lg w-full border border-slate-200 dark:border-slate-700 p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150 my-auto">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-3">
              <h3 className="font-extrabold text-base text-slate-900 dark:text-white flex items-center gap-2">
      ...
      {/* Confirmation Modal to Clear All Agenda Jobs */}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-100 dark:border-slate-700">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              Tem certeza de que deseja apagar <strong>todos os agendamentos</strong> da agenda? Essa ação vai limpar completamente os agendamentos registrados no sistema e no banco de dados para que você possa começar do zero.
            </p>

            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                onClick={() => setIsConfirmClearModalOpen(false)}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl transition-colors"
              >
                Cancelar
              </button>
              <button
              ...
              >
                <Trash2 className="w-3.5 h-3.5" /> Sim, Limpar Tudo
              </button>
            </div>
          </div>
        </div>
