...
/**
 * Geocodes a UK address / postcode into real latitude and longitude coordinates.
 * Priority:
 * 1. Full address (street + city + postcode) via OpenStreetMap / Nominatim API
 * 2. Street + postcode via OpenStreetMap / Nominatim API
 * 3. Postcode centroid fallback via postcodes.io
 */
export async function geocodeUKAddress(
  ...
  const cleanPostcode = postcode ? postcode.trim().toUpperCase() : '';
  const cleanPc = cleanPostcode.replace(/[^A-Z0-9]/g, '');

  let pcLat: number | null = null;
  let pcLng: number | null = null;

  if (cleanPc && cleanPc.length >= 3) {
    try {
      const pcRes = await fetch(`https://api.postcodes.io/postcodes/${encodeURIComponent(cleanPc)}`);
      if (pcRes.ok) {
        const pcData = await pcRes.json();
        if (pcData.result && pcData.result.latitude && pcData.result.longitude) {
          pcLat = pcData.result.latitude;
          pcLng = pcData.result.longitude;
        }
      }
      if (!pcLat) {
        const outcodeRes = await fetch(`https://api.postcodes.io/outcodes/${encodeURIComponent(cleanPc.slice(0, -3) || cleanPc)}`);
        if (outcodeRes.ok) {
          const outData = await outcodeRes.json();
          if (outData.status === 200 && outData.result && outData.result.latitude && outData.result.longitude) {
            pcLat = outData.result.latitude;
            pcLng = outData.result.longitude;
          }
        }
      }
    } catch (e) {
      console.warn('Geocoding postcodes.io warning:', e);
    }
  }

  const isWithinRadius = (lat1: number, lng1: number, lat2: number, lng2: number, maxMiles = 1.5) => {
    const R = 3958.8;
  ...
  if (address && address.trim().length > 0) {
    const fullQuery = [address, city, postcode, 'United Kingdom'].filter(Boolean).join(', ');

    try {
      const nomUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(fullQuery)}&format=json&limit=1`;
      ...
      if (nomRes.ok) {
        ...
        if (Array.isArray(items) && items.length > 0) {
          ...
          if (!isNaN(lat) && !isNaN(lng)) {
            if (!pcLat || !pcLng || isWithinRadius(lat, lng, pcLat, pcLng, 1.5)) {
      ...
      console.warn('Geocoding Nominatim full address warning:', e);
    }
  }

  if (pcLat !== null && pcLng !== null) {
    return { latitude: pcLat, longitude: pcLng };
  }
