...
const DashboardViewComponent: React.FC<DashboardViewProps> = ({
  ...
  const todayRevenue = useMemo(() => todayJobs.reduce((acc, j) => acc + (j.price || 0), 0), [todayJobs]);
  ...
  const estimatedCleanerCost = useMemo(
    () => todayJobs.reduce((acc, j) => acc + j.estimatedDuration * 16, 0),
    [todayJobs]
  );

  const todayProfit = useMemo(
    () => Math.max(0, todayRevenue - estimatedCleanerCost - todayExpenses),
    [todayRevenue, estimatedCleanerCost, todayExpenses]
  );

  const pendingPayments = useMemo(
    () =>
      jobs
        .filter((j) => j.paymentStatus === 'PENDING' || (j.status === 'COMPLETED' && j.paymentStatus !== 'PAID'))
        ...
        <div className="p-5 border-b border-slate-100 dark:border-slate-700/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 dark:bg-slate-800/80">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Interactive map pins showing today’s cleaning sequence, client postcodes, and travel connections.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Navigation className="w-3.5 h-3.5" /> Open Full Route Planner
            </button>
            <a
              href={`https://www.google.com/maps/dir/${todayJobs.map((j) => encodeURIComponent(j.postcode)).join('/')}`}
              target="_blank"
              rel="noreferrer"
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
            {todayJobs.length === 0 ? (
              <div className="p-8 text-center text-slate-400 text-sm">
                No cleaning jobs scheduled for today.
              </div>
            ) : (
                    ...
                    <div className="bg-slate-50/80 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 hover:border-blue-400 transition-all space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-300">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>Cleaner: <strong>{job.cleanerName || 'Assigned Cleaner'}</strong></span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 pt-1 text-[11px]">
                        {job.keyDetails && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                            <Key className="w-3 h-3" /> Key: {job.keyDetails}
                          </span>
                        )}
                        {job.alarmCode && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-red-50 dark:bg-red-950/60 text-red-800 dark:text-red-300 border border-red-200 dark:border-red-800">
                            <ShieldAlert className="w-3 h-3" /> Alarm: {job.alarmCode}
                          </span>
                        )}
                        {job.hasPets && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                            <Dog className="w-3 h-3" /> Pets: {job.petNotes || 'Yes'}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800 flex justify-end gap-2">
                        {job.status === 'SCHEDULED' && (
                          ...
                          >
                            <Play className="w-3.5 h-3.5 fill-current" /> Start Service
                          </button>
                        )}
                        {job.status === 'IN_PROGRESS' && (
                          <button
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[11px] text-slate-500">Revenue vs Net Profit (£)</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-slate-900 to-blue-950 p-5 rounded-3xl text-white border border-slate-800 shadow-md space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-extrabold text-sm">Cleaner Mobile Hub</h4>
                <p className="text-[11px] text-slate-300">GPS Check-in, Photos & Client Signatures</p>
              </div>
            </div>
            <p className="text-xs text-slate-300 leading-relaxed">
              Cleaners can log GPS check-ins, capture before/after photos, and collect client digital signatures directly on mobile.
            </p>
            <button
            ...
            >
              Open Cleaner Mobile Interface <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>
